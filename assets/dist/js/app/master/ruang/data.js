var save_label;
var table;
 $('form#ruang input, form#ruang select').on('change', function () {
...
$(document).ready(function() {
  ...
  table = $("#ruang").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: base_url + "manager/data_ruang/data",
      type: "POST"
      //data: csrf
    },
    columns: [
  ...
  table
    .buttons()
    .container()
    .appendTo("#ruang_wrapper .col-md-6:eq(0)");
  ...
  $("#bulk").on("submit", function(e) {
    if ($(this).attr("action") == base_url + "manager/data_ruang/delete") {
      e.preventDefault();
...
function bulk_delete() {
  if ($("#ruang tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_ruang/delete");
    Swal({
      title: "Anda yakin?",
...
function bulk_edit() {
  if ($("#ruang tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_ruang/edit");
    $("#bulk").submit();
  }
}
