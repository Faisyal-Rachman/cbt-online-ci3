var save_label;
var table;
 $('form#sesi input, form#sesi select').on('change', function () {
...
$(document).ready(function() {
  ...
  table = $("#sesi").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: base_url + "manager/data_sesi/data",
      type: "POST"
      //data: csrf
    },
    columns: [
  ...
  table
    .buttons()
    .container()
    .appendTo("#sesi_wrapper .col-md-6:eq(0)");
  ...
  $("#bulk").on("submit", function(e) {
    if ($(this).attr("action") == base_url + "manager/data_sesi/delete") {
      e.preventDefault();
...
function bulk_delete() {
  if ($("#sesi tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_sesi/delete");
    Swal({
      title: "Anda yakin?",
...
function bulk_edit() {
  if ($("#sesi tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_sesi/edit");
    $("#bulk").submit();
  }
}
