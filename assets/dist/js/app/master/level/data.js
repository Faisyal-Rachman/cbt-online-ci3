...
$(document).ready(function() {
  ...
  table = $("#level").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: base_url + "manager/data_level/data",
      type: "POST"
      //data: csrf
    },
    columns: [
  ...
  $("#bulk").on("submit", function(e) {
    if ($(this).attr("action") == base_url + "manager/data_level/delete") {
      e.preventDefault();
...
function bulk_delete() {
  if ($("#level tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_level/delete");
    Swal({
      title: "Anda yakin?",
...
function bulk_edit() {
  if ($("#level tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_level/edit");
    $("#bulk").submit();
  }
}
