...
$(document).ready(function() {
  ...
  table = $("#kelas").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: base_url + "manager/data_kelas/data",
      type: "POST"
      //data: csrf
    },
    columns: [
  ...
  $("#bulk").on("submit", function(e) {
    if ($(this).attr("action") == base_url + "manager/data_kelas/delete") {
      e.preventDefault();
...
function bulk_delete() {
  if ($("#kelas tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_kelas/delete");
    Swal({
      title: "Anda yakin?",
...
function bulk_edit() {
  if ($("#kelas tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_kelas/edit");
    $("#bulk").submit();
  }
}
