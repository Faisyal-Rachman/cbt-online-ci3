var save_label;
var table;
 $('form#jurusan input, form#jurusan select').on('change', function () {
...
$(document).ready(function() {
  ...
  table = $("#jurusan").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: base_url + "manager/data_jurusan/data",
      type: "POST"
      //data: csrf
    },
    columns: [
  ...
  $("#bulk").on("submit", function(e) {
    if ($(this).attr("action") == base_url + "manager/data_jurusan/delete") {
      e.preventDefault();
...
function bulk_delete() {
  if ($("#jurusan tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_jurusan/delete");
    Swal({
      title: "Anda yakin?",
...
function bulk_edit() {
  if ($("#jurusan tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_jurusan/edit");
    $("#bulk").submit();
  }
}
