var save_label;
var table;
 $('form#guru input, form#guru select').on('change', function () {
...
$(document).ready(function() {
  ...
  table = $("#guru").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: base_url + "manager/data_guru/data",
      type: "POST"
      //data: csrf
    },
    columns: [
  ...
  $("#bulk").on("submit", function(e) {
    if ($(this).attr("action") == base_url + "manager/data_guru/delete") {
      e.preventDefault();
...
function bulk_delete() {
  if ($("#guru tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_guru/delete");
    Swal({
      title: "Anda yakin?",
...
function bulk_edit() {
  if ($("#guru tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "manager/data_guru/edit");
    $("#bulk").submit();
  }
}
