function loadJurusan(id) {
    ...
    $.getJSON(base_url+'jurusanmatpel/getJurusanId/' + id, function (data) {
        console.log(data);
        let opsi;
...
$(document).ready(function () {
    ...
    $('form#jurusanmatpel select').on('change', function () {
        $(this).closest('.form-group').removeClass('has-error');
        $(this).nextAll('.help-block').eq(0).text('');
    });

    $('form#jurusanmatpel').on('submit', function (e) {
        e.preventDefault();
            ...
            success: function (data) {
                ...
                if (data.status) {
                    Swal({
                        ...
                        if (result.value) {
                            window.location.href = base_url+'jurusanmatpel';
                        }
                    });
                } else {
                    if (data.errors) {
