...
class Usermenu_model extends CI_Model{
    ...
    function get_all_menu_no_sma($start, $rows, $search){
		$sql = 'SELECT * FROM user_menu WHERE user_menu.kode_menu NOT LIKE "%"jurusan"%" AND user_menu.kode_menu LIKE "%'.$search.'%" OR user_menu.nama_menu LIKE "%'.$search.'%" ORDER BY tipe ASC, nama_menu ASC LIMIT '.$start.','.$rows;
		return $this->db->query($sql);
	}
