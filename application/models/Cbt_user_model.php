...
class Cbt_user_model extends CI_Model{
 ...
 function count_by_master3($kolom, $isi){
    ...
    function get_by_kolomkartu($kolom, $isi){
        $this->db->select('user_id,user_grup_id,user_name,user_jurusan,tes_jenis,user_sesi,user_password,user_email,user_firstname,user_birthdate,user_detail,user_regdate')
                 ->where($kolom, $isi)
                 ->join('cbt_sesi_ujian', 'cbt_sesi_ujian.sesi_kode = cbt_user.user_sesi')
     ...
     function getidbacara(){
        $sql = "SELECT AUTO_INCREMENT as id FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'cbt' AND TABLE_NAME = 'cbt_bacara'";
