...
class Welcome extends CI_Controller {
	...
	public function index(){
		...
		if ($this->agent->is_browser()){
            if($this->agent->browser()=='Internet Explorer' ){
				...
				$akses_cbt = 1;
				if($this->agent->is_mobile() && $this->agent->browser()=='Opera' || $this->agent->browser()=='Firefox'){
					...
					if($query->row()->konfigurasi_isi=="ya"){
						...
						if(strpos($agent, 'EBC')==false){
							$akses_cbt = 0;
						}
					}
				}else{
					if($this->agent->is_mobile() && $this->agent->browser()=='exambrowser'){
				...
				if($akses_cbt==1){
					if(!$this->access_tes->is_login()){
						...
						$this->template->display_user($this->kelompok.'/welcome_view', 'Peserta Ujian', $data);
					}else{
	...
	function check_login(){	
		...
		$login = $this->access_tes->login($username, $password);
		if($login==1){
			return TRUE;
		}else if($login==2){
			$this->form_validation->set_message('check_login','<small><i class="icon fa fa-info"></i> Password salah / Tes Anda belum Aktif</small>');
			return FALSE;
		}else{
			$this->form_validation->set_message('check_login','Periksa username Anda');
			return FALSE;
		}
	}
}
