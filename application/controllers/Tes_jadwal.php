...
class Tes_jadwal extends Tes_Controller {
	private $kelompok = 'ujian';
	private $url = 'tes_jadwal';
    ...
    public function index(){
        ...
        $this->template->display_tes($this->kelompok.'/tes_jadwal_view', 'Dashboard', $data);
    }
    ...
    if($query_test_nilai_pg = $this->cbt_tes_user_model->get_by_user_tes_pg($temp->tes_id)->row()->tes_status<1){
		...
		if($this->cbt_tes_user_model->count_by_user_tes($user_id, $temp->tes_id)->row()->hasil>0){
			...
			$record[] = '';
			$record[] = '<a class="btn btn-info text-white">Ujian Belum Aktif</a>';
    	...
    	$tgl = date('d M Y',$t);
    	$tgl2 = date('d M Y',$u);
		$record[] = 'Ujian dapat dikerjakan dalam waktu : <b>'.$tgl.' s/d '.$tgl2.'</b>';
