...
class Tes_jadwal extends Tes_Controller {
    ...
    if($query_test_nilai_pg = $this->cbt_tes_user_model->get_by_user_tes_pg($temp->tes_id)->row()->tes_status<1){
    	...
    	$tgl = date('d M Y',$t);
    	$tgl2 = date('d M Y',$u);
		$record[] = 'Waktu ujian : <b>'.$tgl.' s/d '.$tgl2.'</b>';
	            ...
	            if($this->cbt_tes_user_model->count_by_user_tes($user_id, $temp->tes_id)->row()->hasil>0){
	            	...
	            	if($tanggal<$tanggal_tes AND $query_test_user->tesuser_status!=4){
		               ...
		               $st = "<button class='btn btn-danger'>Selesai</button>";
            ...
            <div class="small-box bg-success">
              ...
              <a  href="'.site_url().'unitujian/tes_dashboard/konfirmasi_test/'.$temp->tes_id.'" target="_blank" class="small-box-footer"><h7>Kerjakan <i class="fas fa-arrow-circle-right"></i></h7></a>
            </div>
          </div>';
