...
class Tes_hasil_detail extends Tes_Controller {
 ...
 $tesuser_id = $this->cbt_tes_user_model->get_by_user_id_cek($user_id)->row()->tesuser_id;
        if(!empty($tesuser_id)){
        	...
        	if($query_testuser->num_rows()>0){
				...
				$data['ket'] = $nilai->hasil.'  /  '.$query_test->tes_max_score.'  (nilai / nilai maksimal) ';
				$data['statuspg'] = $query_test->tes_pg;
