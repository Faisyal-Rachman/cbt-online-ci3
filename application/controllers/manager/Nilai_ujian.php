...
foreach ($query2 as $temp) {
	 ...
	 $worksheet->setCellValueByColumnAndRow(2, $row++, $temp->modul_nama.' - '.$temp->nilai.' - '.$temp->tes_nama);
 $worksheet->setCellValueByColumnAndRow(1,3, $temp->tes_jenis.' - '.$temp->grup_nama);
}
...
{
	       $r[] = array('<b>'.$row->modul_nama.' - '.$row->nilai.' - '.$row->tes_jenis.' <small>('.$row->tes_nama.')</small></b>');
