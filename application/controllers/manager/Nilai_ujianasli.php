            ...
            if($query->num_rows()>0){
                ...
                foreach ($query as $temp) {
                	 $worksheet->setCellValueByColumnAndRow(0, $row, $i++);
                	   $query3 = $this->cbt_nilai_model->get_nama_siswa($start, $rows, $tes_id, $grup_id);
			foreach ($query3->result() as $row2)
{
	      $worksht->setCellValueByColumnAndRow(1, $row, $row2->user_birthdate);
	  
}
                      
$query2 = $this->cbt_nilai_model->get_datatable($start, $rows, $tes_id, $grup_id);
...
foreach ($query2 as $temp) {
 ...
 $worksheet->setCellValueByColumnAndRow(1,3, 'Nilai Kelas : '.$temp->grup_nama);
       ...
       $query3 = $this->cbt_nilai_model->get_rata($start, $rows, $tes_id, $grup_id);
			foreach ($query3->result() as $row2)
{
			...
			foreach ($query3->result() as $row2)
{
	      ...
	      //  $record[] =  $row2->user_birthdate;
            $record[] = '<a href="#" title="Klik untuk mengetahui Detail Tes" onclick="detail_tes(\''.$row2->tesuser_id.'\')"><b>'.stripslashes($row2->user_firstname).' - '.$row2->user_birthdate.'</b></a>';
