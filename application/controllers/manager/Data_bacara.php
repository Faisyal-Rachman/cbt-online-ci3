...
class Data_bacara extends Member_Controller {
	...
	public function data()
	{
	  ...
	  function get_absen($modul=null){
        $data['data']=0;
        $data['select_topik'] = '<option value="kosong">Belum ada peserta ujian</option>';
        if(!empty($modul)){
            $data['data']=1;
        $query_topik = $this->cbt_tes_user_model->getTdkHadir('cbt_tes.tes_id', $modul);
            if($query_topik->num_rows()){
                $query_topik = $query_topik->result();
                $select = '';
                foreach ($query_topik as $topik) {
                  
                    $select = $select.'<option value="'.$topik->uid.'">'.$topik->np.' - '.$topik->nl.'</option>';
     ...
     function delete()
	{
		...
		if (!$chk) {
			...
			if ($this->master->delete('cbt_bacara', $chk, 'bacara_id')) {
				$this->master->delete('cbt_absengrup', $chk, 'absen_bacara_id');
