...
class Tes_daftar extends Member_Controller {
    ...
    function update(){
		extract($_POST);
		$topik_id=$id;
		$data['tes_status'] = $status;
		$this->cbt_tes_model->update('tes_id', $topik_id, $data);
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {	
            ...
            $record[] = $temp->tes_nama.' <br><b>Mapel : '.$temp->modul_nama.'</b>';
			$record[] = $temp->modul_kkm;
			$record[] = $temp->tes_jenis;
            $record[] = $a.'  '.$akh;
            $record[] = $temp->tes_duration_time.' Mnt';
			...
			foreach ($query2->result() as $row)
{
		...
		//	}
		$record[] = '<b data="'.$temp->tes_id.'" class="status_checks btn '.(($temp->tes_status)?'btn-success btn-sm' : 'btn-danger btn-sm').'">
			'.(($temp->tes_status)?'Aktif' : 'Nonaktif').'</b>';
