...
class Tes_hasil extends Member_Controller {
    ...
    public function index($page=null, $id=null){
        ...
        $select = '<option value="semua">Semua Ujian</option>';
           ...
           $user_id = $this->input->post('user_id', TRUE);
            if($pilihan=='hapus'){
                foreach( $tesuser_id as $kunci => $isi ) {
                 ...
                 $status['siswa'] = $user_id;
            	$status['status'] = 1;
            	$status['pesan'] = 'telah dihapus dari sesi Ujian.';
            	...
            	foreach( $tesuser_id as $kunci => $isi ) {
                  ...
                  $status['siswa'] = $user_id;
            	$status['status'] = 1;
            	$status['pesan'] = 'di blok dari Ujian.';
            	$status['pesan2'] = 'Karena melanggar tata tertib Ujian.';
            	...
            	foreach( $tesuser_id as $kunci => $isi ) {
                 ...
                 $status['siswa'] = $user_id;
            	$status['status'] = 1;
            	$status['pesan'] = 'bisa mengerjakan Ujian kembali';
            	$status['pesan2'] = 'Harap tetap menjaga tata tertib!';
    ...
    function export($tes_id=null, $grup_id=null, $waktu=null, $urutkan=null, $status=null){
        if(!empty($tes_id) AND !empty($grup_id) AND !empty($waktu) AND !empty($urutkan) AND !empty($status)){
			...
			if($status=='mengerjakan'){
				...
				$query = $this->cbt_user_model->get_by_tes_group_urut_tanggal($tes_id, $grup_id, $urutkan, $tanggal);
			}
            ...
            if($query->num_rows()>0){
                ...
                foreach ($query as $temp) {
                    ...
                    $worksheet->setCellValueByColumnAndRow(4, $row, stripslashes($temp->user_firstname).stripslashes($temp->user_birthdate));
	...
	//	$keterangan = $this->input->get('keterangan');
		...
		if($status=='mengerjakan'){
			...
			$query = $this->cbt_user_model->get_datatable_hasiltes($start, $rows, $tes_id, $grup_id, $urutkan, $tanggal);
			$iTotal= $this->cbt_user_model->get_datatable_hasiltes_count($tes_id, $grup_id, $urutkan, $tanggal)->row()->hasil;
		}
	    ...
	    foreach ($query as $temp) {			
			...
			if(empty($temp->tesuser_id)){
				$record[] = '<b>'.stripslashes($temp->user_firstname).stripslashes($temp->user_birthdate).'</b>';
			}else{
				$record[] = '<a href="#" title="Klik untuk mengetahui Detail Tes" onclick="detail_tes(\''.$temp->tesuser_id.'\')"><b>'.stripslashes($temp->user_firstname).' '.stripslashes($temp->user_birthdate).'</b></a>';
			}
			if(empty($temp->nilai)){
				$record[] = '0';
			}else{
			...
			if(empty($temp->tesuser_id)){
				...
				$record[] = '<input type="checkbox" name="edit-testuser-id['.$temp->tesuser_id.']" ><input type="hidden" name="user_id" value="'.$temp->user_birthdate.'" >';
			}
