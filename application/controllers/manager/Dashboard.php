...
class Dashboard extends Member_Controller {
	...
	function status(){
		...
		$cs = $this->cbt_tes_user_model->tampilstatus();
		...
		<div class="text-dark">
		  <span class="info-box-icon">'.$us.' '.$cstat['user_birthdate'].'</span>

		  <div class="info-box-content">
			<span class="info-box-text">'.$stat.' </span>
			<div class="progress">
			  <div class="progress-bar text-light" style="width: '.$bar.'%"></div>
			</div>
	...
	function statusujian(){
		...
		if($rows>0){
			$hd = '<table class="table table-bordered table-striped table-sm">
	...
	$output = '
<tr> 
<td width="220px">'.$cstat['tes_nama'].'</td>
<td>'.$cstat['modul_nama'].'</td>
<td width="100px">'.$cstat['tesuser_token'].'</td>
<td width="100px"><a onclick="status()" target="_blank" class="btn btn-warning btn-sm" style="cursor: pointer;">Lihat User</a>
</td>
</tr>
';
...
<tr>
<th colspan="5">Peringkat 10 Besar</th>
</tr>
</thead>
		<thead class="table table-info">
<tr>
