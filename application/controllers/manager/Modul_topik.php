...
class Modul_topik extends Member_Controller {
	...
	function update(){
		extract($_POST);
		$topik_id=$id;
		$data['topik_aktif'] = $status;
		$this->cbt_topik_model->update('topik_id', $topik_id, $data);
    ...
    function edit(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($pilihan=='hapus'){//hapus
				...
				$data['guru_pengampu'] = $this->input->post('edit-deskripsi', true);
                ...
                if($topik_asli!=$data['topik_nama']){
                	...
                	$this->cbt_topik_model->update('topik_id', $id, $data);
                	$status['status'] = 1;
                	$status['pesan'] = 'Topik Berhasil diubah';
                }
            }
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			$jml_esai = 0;
			$record[] = ++$i;
			$jml_soal = $this->cbt_soal_model->count_by_kolom('soal_topik_id', $temp->topik_id)->row();
			
			$jml_pilganda = $this->cbt_soal_model->count_by_kolomsoal('soal_topik_id', $temp->topik_id)->row();
		//	$jml_js = $this->cbt_soal_model->count_by_kolomsoal('soal_topik_id', $temp->topik_id)->row();
		//	$jml_esai = $this->cbt_soal_model->count_by_kolomsoal('soal_topik_id', $temp->topik_id)->row();
		$obj='';
		$js='';
		$essai='';
		if (empty($jml_pilganda->objective) && empty($jml_pilganda->js) && empty($jml_pilganda->js)) {
			$obj='0';
			$js='0';
			$esai='0';
		  }else{
			$obj = $jml_pilganda->objective;
			$js = $jml_pilganda->js;
			$esai = $jml_pilganda->esai;
		  }
            $record[] = $temp->topik_nama;
            $record[] = $temp->guru_pengampu;
			$record[] = $jml_soal->hasil.'-('.$obj.' Objective | '.$js.' Jawab singkat | '.$esai.' Essai)';
			$record[] = '<b data="'.$temp->topik_id.'" class="status_checks btn '.(($temp->topik_aktif)?'btn-success btn-sm' : 'btn-danger btn-sm').'">
			'.(($temp->topik_aktif)?'Aktif' : 'Nonaktif').'</b>';
		   /*
			...
			*/
			$record[] = '<table align="center"><tr><td><form action="'.site_url().'manager/modul_import" method="post" target="_blank"><input type="hidden" name="idtopik" value="'.$temp->topik_id.'"><input type="hidden" name="nmtopik" value="'.$temp->topik_nama.'">
				<button type="submit" class="btn btn-info btn-sm">Import</button></form></td><td>
				<form action="'.site_url().'manager/modul_soal" method="post" target="_blank"><input type="hidden" name="idtopik" value="'.$temp->topik_id.'"><input type="hidden" name="nmtopik" value="'.$temp->topik_nama.'">
				<button type="submit" class="btn btn-success btn-sm"><i class="fas fa-bars"></i></button></form></td><td><a onclick="edit(\''.$temp->topik_id.'\')" style="cursor: pointer;" class="btn btn-warning btn-sm">Edit</a></td></tr></table>';
