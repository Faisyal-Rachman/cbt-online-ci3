...
class Rangking extends Member_Controller {
  private $kode_menu = 'rangking';
  private $kelompok = 'nilai';
  private $url = 'manager/rangking';
...
$data['th'] ='no';
  ...
  $data['th'] ='table-hasil';
        ...
        $query_group = $this->cbt_tes_user_model->get_by_jurusan();
        $select = '<option value="semua">Pilih Jurusan</option>';
        if($query_group->num_rows()>0){
          $query_group = $query_group->result();
          foreach ($query_group as $temp) {
            $select = $select.'<option value="'.$temp->jurusan_kode.'">'.$temp->jurusan_nama.'</option>';
          }
        ...
        $this->template->display_admin($this->kelompok.'/rangking_view', 'Hasil Tes', $data);
    }
      ...
      foreach ($query3->result() as $row2)
{
  ...
  //  $keterangan = $this->input->get('keterangan');
    ...
    // run query to get user listing
      ...
      $query = $this->cbt_nilai_model->get_nama_rangking($grup_id);
      $iTotal= $this->cbt_nilai_model->get_datatable_countrang($grup_id)->row()->hasil;
   ...
   foreach ($query as $temp) {      
    ...
    $t = array(); 
    $record = array();
$query3 = $this->cbt_nilai_model->get_ratarangking($grup_id,$temp->user_birthdate);
 foreach ($query3->result() as $row3)
{
$cnv = number_format($row3->rata,1,',',',');

if($status=='peringkat1' and $cnv>=85 and $cnv<=89){
$ptk = 'Peringkat 1';
 $record[] = $temp->user_birthdate;
 $record[] = $temp->user_jurusan;
 $record[] = $cnv;
 $record[] = $ptk;
}elseif($status=='peringkat2' and $cnv>=90 and $cnv<=94){
   $ptk = 'Peringkat 2';
  $record[] = $temp->user_birthdate;
  $record[] = $temp->user_jurusan;
  $record[] = $cnv;
  $record[] = $ptk;
}elseif($status=='peringkat3' and $cnv>=95 and $cnv<=100){
$ptk = 'Peringkat 3';
 $record[] = $temp->user_birthdate;
  $record[] = $temp->user_jurusan;
  $record[] = $cnv;
