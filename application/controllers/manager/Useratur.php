...
class Useratur extends Member_Controller {
    ...
    public function index($page=null, $id=null){
        ...
        if($page=="add"){
            ...
            $this->master->guru_pengampu2();
            $query =  $this->master->guru_pengampu2();
            $level_opsi = '';
            if($query->num_rows()>0){
                $level_opsi = $level_opsi.'<select disabled name="nama" id="level" class="form-control input-sm hide">';
                $level = $query->result();
                foreach($query->result() as $temp){
                    $level_opsi = $level_opsi.'
                        <option value="'.$temp->guru_nama.'">'.$temp->guru_nama.'</option>
                    ';
                }
            ...
            if(empty($id)){
                ...
                if($query_user->num_rows()>0){
                    ...
                    $level_opsi = '';
                    if($query->num_rows()>0){
                        $level_opsi = $level_opsi.'<select name="nama" id="level" class="form-control input-sm">';
                        $level = $query->result();
    ...
    function edit(){
        ...
        $this->form_validation->set_rules('tambah-tipe', 'Level','required|strip_tags');
    ...
    function add(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($this->users_model->get_login_info($username)){
                ...
                if($this->input->post('tambah-tipe') == 'guru'){
                ...
                $data_user['nama'] = $this->input->post('nama', true);
                $data_user['level'] = $this->input->post('tambah-tipe', true);
    ...
    function get_all_user(){
	    ...
	    foreach ($user as $temp) {			
            ...
            if($is_edit){
                $record[] = '<a href="'.site_url('manager/useratur/index/edit/'.$temp->id).'"  class="btn btn-warning btn-sm">Edit</a>';
            }else{
                $record[] = '';
            }
