    ...
    if($jenjang='sma'){
        $op1 = 5;
          ...
          $data2['tset_jawaban'] = $op1;
            ...
            if($cek_nama==1 and $this->cbt_tes_model->count_by_kolom('tes_nama', $data['tes_nama'])->row()->hasil>0 or $this->cbt_tes_user_model->count_by_kolom('tesuser_tes_id', $data2['tset_tes_id'])->row()->hasil>0){
            	$status['status'] = 0;
            	$status['pesan'] = 'Tes Masih digunakan, Tidak bisa mengubah.';
            }else{
                $is_process = 1;
            	// Menyimpan data tes
            	if(empty($tes_id)){
                ...
                if($is_process==1){
                   ...
                   $this->cbt_tesgrup_model->delete('tstgrp_tes_id', $tes_id);
                    ...
                    if(!empty($tes_id)){
                        ...
                        $status['pesan'] = 'Tes berhasil disimpan, Lanjutkan setting Soal';
                    }else{
                        $status['status'] = 1;
                        $status['tes_id'] = $tes_id;
                        $status['pesan'] = 'Tes berhasil ditambahkan, Lanjutkan setting Soal ya';
                    }
                }
            }
     //   $modulid = $this->cbt_tes_topik_set_model->get_tessoal($tes_id);
