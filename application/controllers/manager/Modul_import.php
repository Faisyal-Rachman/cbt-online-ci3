...
class Modul_import extends Member_Controller {
            ...
            if(!empty($query_user->opsi1)){
                ...
                foreach ($user_topik as $topik_id) {
                    ...
                    if($query_topik->num_rows()>0){
                        ...
                        $select2 = $select2.'<option value="'.$topikid.'" selected>'.$topik->modul_nama.' - '.$topiknm.' ['.$jml_soal.']</option>';
                    }
                }
            }else{
                ...
                if($query_modul->num_rows()>0){
                    ...
                    foreach ($query_modul as $temp) {
                        ...
                        if($query_topik->num_rows()){
                            ...
                            $select2 = $select2.'<optgroup label="Pelajaran Auto '.$temp->modul_nama.'">';
    ...
    function import(){
        ...
        if($this->form_validation->run() == TRUE){
	        ...
	        if(!empty($_FILES['userfile']['name']) or ($_FILES['userfile2']['name'])){
    ...
    function import_file($inputfile, $id_topik){
        ...
        if($highestRow>10){
            ...
            while($kosong<2){
                $kosong=0;
                $kolom1 = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
                $kolom2 = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
                $kolom3 = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
                 $kolom5 = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
                $kolom4 = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
                 $kolom6 = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
                ...
                if($kosong==0){
                  ...
                  if($kolom1=='QE'){
                     ...
                     if($kolom1=='QJS'){
                        $question['soal_topik_id'] = $id_topik;
                        $question['soal_detail'] = $kolom2;
                        $question['soal_tipe'] = '3';
                         $question['soal_kunci'] = $kolom5;
                        $question['soal_difficulty'] = $kolom4;
                        $question['soal_aktif'] = 1;
                         $question['soal_audio'] = $kolom6;
