...
class Tes_status extends Member_Controller {
	private $kode_menu = 'tes-status';
	private $kelompok = 'tes';
	private $url = 'manager/tes_status';
    ...
    public function index($page=null, $id=null){
		$datajson = $this->get_datatable();
		//$datagetjson = file_get_contents($datajson);
		$data = json_decode($datajson, true);
		$data = [
			'jenis'	=> $this->master->jenis(),
			'ruangan'	=> $this->master->ruang(),
			'matpel'	=> $datajson
	];
	...
	$data['url'] = $this->url;
        ...
        $this->template->display_admin($this->kelompok.'/tes_status_view', 'Hasil Tes', $data);
    }
...
/*
		...
		if($status=='mengerjakan'){
			$query = $this->cbt_tes_user_model->get_datatable($start, $rows, $tes_id, $grup_id, $jenis, $urutkan, $tanggal, $keterangan);
			$iTotal= $this->cbt_tes_user_model->get_datatable_count($tes_id, $grup_id, $urutkan, $tanggal, $keterangan)->row()->hasil;
		}elseif($status=='tidak'){
			$query = $this->cbt_user_model->get_datatable_hasiltes($start, $rows, $tes_id, $grup_id, $jenis, $urutkan, $tanggal, $keterangan);
			$iTotal= $this->cbt_user_model->get_datatable_hasiltes_count($tes_id, $grup_id, $urutkan, $tanggal, $keterangan)->row()->hasil;
		}else{
			echo "TIDAK BISA LOAD DATA";
		}
