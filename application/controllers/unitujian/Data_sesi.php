...
class Data_sesi extends Member_Controller {
	...
	{
		$rows = count($this->input->post('sesi_nama', true));
		$mode = $this->input->post('mode', true);
		for ($i = 1; $i <= $rows; $i++) {
			//ini inisialisasi var
			$sesi_nama = 'sesi_nama';
			$sesi_kode = 'sesi_kode';
			$this->form_validation->set_rules($sesi_nama, 'Nama Sesi', 'required');
			$this->form_validation->set_rules($sesi_kode, 'Sesi Kode', 'required');
			$this->form_validation->set_message('required', '{field} Wajib diisi');

			if ($this->form_validation->run() === FALSE) {
				$error[] = [
				
					$sesi_nama => form_error($sesi_nama)
				];
				$status = FALSE;
			} else {
				if ($mode == 'add') {
					$insert[] = [
						'sesi_nama' => $this->input->post($sesi_nama, true),
						'sesi_kode' => $this->input->post($sesi_kode, true)
					];
		...
		if ($status) {
			if ($mode == 'add') {
				$this->master->create('cbt_sesi_ujian', $insert, true);
				$data['insert']	= $insert;
			} else if ($mode == 'edit') {
				$this->master->update('cbt_sesi_ujian', $update, 'sesi_id', null, true);
				$data['update'] = $update;
			}
		} else {
			if (isset($error)) {
	...
	public function save()
	{
		...
		for ($i = 1; $i <= $rows; $i++) {
			//ini inisialisasi var
			$sesi_nama = 'sesi_nama[' . $i . ']';
			$sesi_kode = 'sesi_kode[' . $i . ']';
			$this->form_validation->set_rules($sesi_nama, 'Nama Sesi', 'required');
			$this->form_validation->set_rules($sesi_kode, 'Sesi Kode', 'required');
			$this->form_validation->set_message('required', '{field} Wajib diisi');
			...
			if ($this->form_validation->run() === FALSE) {
				$error[] = [
					$sesi_nama => form_error($sesi_nama),
					$sesi_kode => form_error($sesi_kode),
				];
				$status = FALSE;
			} else {
				if ($mode == 'add') {
					...
					$update[] = array(
						'sesi_id'	=> $this->input->post('sesi_id[' . $i . ']', true),
						'sesi_nama' 	=> $this->input->post($sesi_nama, true),
						'sesi_kode' 	=> $this->input->post($sesi_kode, true)
    ...
    function get_by_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query = $this->cbt_user_grup_model->get_by_kolom('sesi_id', $id);
			if($query->num_rows()>0){
				$query = $query->row();
				$data['data'] = 1;
				$data['id'] = $query->sesi_id;
				$data['group'] = $query->sesi_nama;
			}
		}
    ...
    function get_datatable(){
		...
		$query = $this->cbt_user_grup_model->get_datatable($start, $rows, 'sesi_nama', $search);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->cbt_user_grup_model->get_datatable_count('sesi_nama', $search)->row()->hasil;
	    ...
	    foreach ($query as $temp) {			
            ...
            $record[] = '<a onclick="edit(\''.$temp->sesi_id.'\')" style="cursor: pointer;" class="btn btn-default btn-xs">Edit</a>';
