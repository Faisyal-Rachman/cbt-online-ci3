...
class Tes_kerjakan extends Tes_Controller {
	...
	private $url = 'unitujian/tes_kerjakan';
    private $username;
    private $user_id;
    ...
    public function index($tes_id=null){
        ...
        if(isset($active_time)) {
          ...
          if(!empty($tes_id)){
            ...
            $data['url'] = $this->url;
            $data['id'] =  $this->access_tes->get_userid();
            ...
            if($query_tes->num_rows()>0){
                ...
                if($tanggal>=$tanggal_tes){
                    ...
                    $data['btnselesai'] = $query_tes->tes_pg;
                    ...
                    $data_soal = $this->get_soal($tessoal->tessoal_id, $query_tes->tesuser_id, $tes_id);
                    ...
                    $this->template->display_tes_mulai($this->kelompok.'/tes_kerjakan_view', 'Kerjakan Tes', $data);
                }
            }else{
                redirect('tes_dashboard');
            }
    ...
    //$j = $this->cbt_tes_soal_jawaban_model->get_by_tessoal_answer($tes_soal_id, $jawaban)->row();
                ...
                if($query_soal->num_rows()>0){
                    ...
                    if($query_soal->soal_tipe==1){
                        // Mendapatkan data tes
                          $status['jwbn'] = $this->cbt_tes_soal_jawaban_model->get_tessoal_jawab($tes_soal_id, $jawaban)->row()->hasil;
...
//var_dump($abjad);
                        ...
                        $status['status'] = 1;
                        $status['nomor_soal'] = $tes_soal_nomor;
                        $status['pesan'] = 'Jawaban essay disimpan';
  ...
  function get_tes_jawab($tes_id=null){
        ...
        //    $cdbremidi = $this->cbt_tes_model->del_remidi();
            ...
            if($query_tes->num_rows()>0){
                ...
                $data['tes_user_id'] = $query_tes->tesuser_id;
                $data['tes_nama'] = $query_tes->tes_nama;
                 $data['tes_cekjawab'] = $this->cbt_tes_soal_model->count_by_tesuser_dijawab($query_tes->tesuser_id)->row()->hasil.' Soal';
                $data['tes_dijawab'] = $this->cbt_tes_soal_model->count_by_tesuser_dijawab($query_tes->tesuser_id)->row()->hasil.' Soal';
                $data['tes_blum_dijawab'] = $this->cbt_tes_soal_model->count_by_tesuser_blum_dijawab($query_tes->tesuser_id)->row()->hasil.' Soal';
            }
        }
     ...
     */
    function get_soal_by_tessoal($tessoal_id=null, $tesuser_id=null, $tes_id=null){
        $data['data'] = 0;
        if(!empty($tessoal_id) AND !empty($tesuser_id)){
            $data_soal = $this->get_soal($tessoal_id, $tesuser_id, $tes_id);
                    ...
                    foreach ($query_soal as $soal) {
                        ...
                        if(!empty($soal->tessoal_change_time)){
                            if($soal->tessoal_ragu==0){
                                ...
                                $data_soal = $data_soal.'<button id="btn-soal-'.$soal->tessoal_order.'" onclick="soal(\''.$soal->tessoal_id.'\')" class="btn nav-link btn-primary" style="margin-bottom: 5px;" title="Soal ke '.$soal->tessoal_order.'">'.$soal->tessoal_order.' <span class="badge navbar-badge badge-light" id="bjb'.$soal->tessoal_order.'"></span></button>
                                ';
                            }else{
                                // Jika soal ragu-ragu
                            ...
                            if($soal->tessoal_ragu==0){
                                // Jika soal tidak ragu-ragu
                                $data_soal = $data_soal.'<button id="btn-soal-'.$soal->tessoal_order.'" onclick="soal(\''.$soal->tessoal_id.'\')" class="btn nav-link btn-default" style="margin-bottom: 5px;" title="Soal ke '.$soal->tessoal_order.'">'.$soal->tessoal_order.' <span class="badge navbar-badge badge-light" id="bjb'.$soal->tessoal_order.'"></span></button>
     ...
     */
    private function get_soal($tessoal_id=null, $tesuser_id=null, $tes_id=null){
        $data['tes_soal_id'] = '';
        $data['tes_soal'] = '';
        $data['data'] = 0;
        if(!empty($tessoal_id) AND !empty($tesuser_id)){
            ...
            if($this->cbt_tes_user_model->count_by_status_waktuuser($tesuser_id, $waktuuser)->row()->hasil>0){
               ...
               $query_audio = $this->cbt_tes_soal_model->get_by_soalaudio_limit($tes_id, 1);
                $soal = '';
                if($query_soal->num_rows()>0){
                    $data['tes_soal_id'] = $tessoal_id;

                    $query_soal = $query_soal->row();
                  $query_audio = $query_audio->row();
                    // Soal Ragu-ragu
                    ...
                    if(!empty($query_soal->soal_audio)){
                        $audio_play = 0;
                        $putar = '';
                        if($query_audio->tes_audio_play==1){
                            $audio_play = 1;
                            $putar = '<small>*Audio dimainkan hanya sekali.</small>';
                        }
                        // jika batasan play audio masih bernilai 0
                        if($query_soal->tessoal_audio_play==0){
                        
                            $tm = $this->cbt_topik_model->get_by_kolom_join_modul('topik_id', $query_soal->soal_topik_id);
         $tm = $tm->result();
                foreach ($tm as $tmp) {
$tmn = $tmp->modul_nama;
$tmnt = $tmp->topik_nama;
                        ...
                        if($query_jawaban->num_rows()>0){
                            ...
                            foreach ($query_jawaban as $jawaban) {
                                ...
                                if($jawaban->soaljawaban_selected==1){
                                    $soal = $soal.'<div id="ck-button"><label><input type="radio" onchange="jawab()" name="soal-jawaban" value="'.$jawaban->soaljawaban_jawaban_id.'" checked><span class="jwb">'.$b.'</span></label></div><p class="notxtjwb">'.$temp_jawaban.'</p><p><br></p>';
                                }else{
                                    $soal = $soal.'<div id="ck-button"><label><input type="radio" onchange="jawab()" name="soal-jawaban" value="'.$jawaban->soaljawaban_jawaban_id.'" ><span class="jwb">'.$b.'</span></label></div><p class="txtjwb">'.$temp_jawaban.'</p><p><br></p>';
                                } 
                                $i++;
                            }
                        }
