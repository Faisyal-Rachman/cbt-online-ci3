...
class Data_jenis extends Member_Controller {
	...
	{
		$rows = count($this->input->post('jenis_nama', true));
		...
		for ($i = 1; $i <= $rows; $i++) {
			...
			$this->form_validation->set_rules($jenis_nama, 'Nama jenis', 'required');
			$this->form_validation->set_message('required', '{field} Wajib diisi');

			if ($this->form_validation->run() === FALSE) {
				$error[] = [
					$jenis_nama => form_error($jenis_nama)
				];
				$status = FALSE;
			} else {
				if ($mode == 'add') {
					$insert[] = [
						'jenis_nama' => $this->input->post($jenis_nama, true),
		...
		if ($status) {
			if ($mode == 'add') {
				$this->master->create('cbt_jenis_ujian', $insert, true);
				$data['insert']	= $insert;
			} else if ($mode == 'edit') {
				$this->master->update('cbt_jenis_ujian', $update, 'jenis_id', null, true);
				$data['update'] = $update;
			}
		} else {
			if (isset($error)) {
	...
	public function save()
	{
		...
		for ($i = 1; $i <= $rows; $i++) {
			//ini inisialisasi var
			$jenis_nama = 'jenis_nama[' . $i . ']';
			$kode_jenis = 'kode_jenis[' . $i . ']';
			$status = 'status';
			...
			if ($this->form_validation->run() === FALSE) {
				$error[] = [
					$jenis_nama => form_error($jenis_nama),
					$kode_jenis => form_error($kode_jenis),
				];
				$status = FALSE;
			} else {
				if ($mode == 'add') {
					...
					$update[] = array(
						'jenis_id'	=> $this->input->post('jenis_id[' . $i . ']', true),
						'jenis_nama' => $this->input->post($jenis_nama, true),
						'kode_jenis' => $this->input->post($kode_jenis, true),
						'status' => $this->input->post($status, true)
	...
	public function delete()
	{
		...
		if (!$chk) {
			...
			if ($this->master->delete('cbt_jenis_ujian', $chk, 'jenis_id')) {
	...
	public function edit()
	{
		...
		if (!$chk) {
			...
			$this->template->display_admin($this->kelompok.'/jenis/edit', 'Daftar Jenis Ujian', $data);
