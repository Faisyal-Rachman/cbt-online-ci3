...
class Data_mapel extends Member_Controller {
	...
	public function save()
	{
		...
		for ($i = 1; $i <= $rows; $i++) {
			//ini inisialisasi var
			$modul_nama = 'modul_nama[' . $i . ']';
			$modul_aktif = 'modul_aktif[' . $i . ']';
			$this->form_validation->set_rules($modul_nama, 'Mata Kuliah', 'required');
			$this->form_validation->set_message('required', '{field} Wajib diisi');
			...
			if ($this->form_validation->run() === FALSE) {
				...
				if ($mode == 'add') {
					...
					$update[] = array(
						'modul_id'	=> $this->input->post('modul_id[' . $i . ']', true),
						'modul_nama' 	=> $this->input->post($modul_nama, true),
						'modul_aktif' 	=> $this->input->post($modul_aktif, true)
					);
				}
	...
	public function edit()
	{
		...
		if (!$chk) {
			...
			$data = [
				...
				'subjudul'	=> 'Edit Data Matpel',
				'matpel'	=> $matpel
			];
		
			$this->template->display_admin($this->kelompok.'/matpel/edit', 'Daftar Group', $data);
