    ...
    function export($tes_id=null, $grup_id=null, $waktu=null, $urutkan=null, $status=null, $keterangan=null){
        if(!empty($tes_id) AND !empty($grup_id) AND !empty($waktu) AND !empty($urutkan) AND !empty($status)){
			...
			if($status=='mengerjakan'){
				$query = $this->cbt_tes_user_model->get_by_tes_group_urut_tanggal_absen($tes_id, $grup_id, $urutkan, $tanggal);
			}else{
				$query = $this->cbt_user_model->get_by_tes_group_urut_tanggal_absen($tes_id, $grup_id, $urutkan, $tanggal);
			}
            ...
            if($query->num_rows()>0){
                ...
                foreach ($query as $temp) {
					$a = date('Y-m-d', strtotime($temp->tes_begin_time));
					$b = date('Y-m-d', strtotime($temp->tes_end_time));
					if ($temp->tesuser_creation_time>0){
						$lg = $temp->tesuser_creation_time;
					}else{
						$lg = "Tidak login";
					}
					if ($temp->grup_nama>0){
                    ...
                    $worksheet->setCellValueByColumnAndRow(1, $row, $lg);
					$worksheet->setCellValueByColumnAndRow(2, $row, $temp->tes_nama);
				    $worksheet->setCellValueByColumnAndRow(4, $row, stripslashes($temp->user_birthdate));
                    $worksheet->setCellValueByColumnAndRow(5, $row, $rg);
					$worksheet->setCellValueByColumnAndRow(3, $row, $temp->user_name);
					$worksheet->setCellValueByColumnAndRow(6, $row, $a.' - '.$b);
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			if(empty($temp->tesuser_id)){
				$record[] = '<b>'.stripslashes($temp->user_birthdate).'</b>';
			}else{
