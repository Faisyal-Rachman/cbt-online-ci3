...
$exp = date('Y-m-d',strtotime($waktu));
	...
	function hapus_daftar_token(){
    	$this->load->library('form_validation');
        
		$this->form_validation->set_rules('token-id[]', 'Token','required|strip_tags');
		if($this->form_validation->run() == TRUE){
			$token_id = $this->input->post('token-id', TRUE);
			foreach( $token_id as $kunci => $isi ) {
				if($isi=="on"){
					$this->cbt_tes_token_model->delete('token_id', $kunci);
            	}
            }
            $status['status'] = 1;
            $status['pesan'] = 'Data Token berhasil dihapus';
		}else{
			$status['status'] = 0;
            $status['pesan'] = validation_errors();
        }
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			$record[] = '<input type="checkbox" name="token-id['.$temp->token_id.']" >';
