...
class Tes_sudah_ujian extends Tes_Controller {
	private $kelompok = 'ujian';
	private $url = 'tes_sudah_ujian';
    ...
    public function index(){
        ...
        $data['sdhujian'] = $this->cbt_tes_user_model->get_by_user_status_cek($user_id);
        ...
        $this->template->display_tes($this->kelompok.'/tes_sudah_ujian_view', 'Dashboard', $data);
    }
	function get_by_id($id=null){
		...
		if(!empty($id)){
			$query = $this->master->getInfobyid($id);
				...
				$data['data'] = 1;
				$data['id'] = $row->info_id;
				$data['judul'] = $row->info_judul;
				$data['isi'] = $row->info_isi;
				$data['kategori'] = $row->info_kategori;
				$data['tgl'] = $row->info_tgl;
				}
    ...
    if($query_test_nilai_pg = $this->cbt_tes_user_model->get_by_user_tes_pg($temp->tes_id)->row()->tes_status<1){
		...
		$output['aaData'][] = $record;
			...
			//	$record[] = ++$i;
	           $t = strtotime($temp->tes_begin_time);
	           $u = strtotime($temp->tes_end_time);
    	$tgl = date('d M Y',$t);
    	$tgl2 = date('d M Y',$u);
		$record[] = 'Ujian dapat dikerjakan dalam waktu : <b>'.$tgl.' - '.$tgl2.'</b>';
	            ...
	            if($this->cbt_tes_user_model->count_by_user_tes($user_id, $temp->tes_id)->row()->hasil>0){
	            	...
	            	if($tanggal<$tanggal_tes AND $query_test_user->tesuser_status!=4){
	            		...
	            		$record[] = '';
	            		// Jika masih dalam waktu pengerjaan, maka tes dilanjutkan
	            		$record[] = '<span class="badge badge-success"> <h6>Sedang dikerjakan!</h6> </span>';
	            	}else{
	            		
		            	if($temp->tes_results_to_users==1){
							
		               $st = "<button class='btn btn-warning'>Selesai</button>";
	            		...
	            		// mengecek apakah detail tes ditampilkan
	            		if($temp->tes_detail_to_users==1){
	            			$record[] = '<a href="'.site_url().'/tes_hasil_detail/index/'.$query_test_user->tesuser_id.'" style="cursor: pointer;" class="btn btn-success">Lihat Detail</a>';
	            		}else{
	            			$record[] = '';
	            		}
	            	}
	            	...
	            	$record[] = '';
	            	$record[] = ' <div class="col-lg-3 col-6">
            <!-- small box -->
            <div class="small-box bg-info">
              <div class="inner">
               
                          '.$temp->tes_jenis.' '.$temp->modul_nama.' '.$temp->tes_nama.'
              ...
              <a  href="'.site_url().'unitujian/tes_dashboard/konfirmasi_test/'.$temp->tes_id.'" target="_blank" class="small-box-footer"><h5>Kerjakan <i class="fas fa-arrow-circle-right"></i></h5></a>
            </div>
          </div>';
